...
namespace ostgui
{
    ...
    [ContextClass("ТфРамкаГруппы", "TfFrameView")]
    public class TfFrameView : AutoContext<TfFrameView>
    {
        ...
        public IValue Focused
        {
            get
            {
                if (Base_obj.M_FrameView.Focused.GetType().ToString().Contains("+ContentView"))
                {
                    return Utils.RevertEqualsObj(Base_obj.M_FrameView.Focused.Focused).dll_obj;
                }
                return Utils.RevertEqualsObj(Base_obj.M_FrameView.Focused).dll_obj;
            }
        }
        ...
        public IValue Y
        {
            ...
            set
            {
                ...
                {
                    Base_obj.M_FrameView.Y = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_FrameView.Y = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        public IValue X
        {
            ...
            set
            {
                if (Utils.IsType<TfPos>(value))
                {
                    Base_obj.M_FrameView.X = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_FrameView.X = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        public IValue SuperView
        {
            get
            {
                if (Base_obj.M_FrameView.SuperView.GetType().ToString().Contains("+ContentView"))
                {
                    return Utils.RevertEqualsObj(Base_obj.M_FrameView.SuperView.SuperView).dll_obj;
                }
                return Utils.RevertEqualsObj(Base_obj.M_FrameView.SuperView).dll_obj;
            }
        }
