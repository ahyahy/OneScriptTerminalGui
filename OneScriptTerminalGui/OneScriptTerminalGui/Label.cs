...
namespace ostgui
{
    ...
    [ContextClass("ТфНадпись", "TfLabel")]
    public class TfLabel : AutoContext<TfLabel>
    {
        ...
        public IValue Y
        {
            ...
            set
            {
                ...
                {
                    Base_obj.M_Label.Y = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_Label.Y = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        public IValue X
        {
            ...
            set
            {
                if (Utils.IsType<TfPos>(value))
                {
                    Base_obj.M_Label.X = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_Label.X = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        public IValue SuperView
        {
            get
            {
                if (Base_obj.M_Label.SuperView.GetType().ToString().Contains("+ContentView"))
                {
                    return Utils.RevertEqualsObj(Base_obj.M_Label.SuperView.SuperView).dll_obj;
                }
                return Utils.RevertEqualsObj(Base_obj.M_Label.SuperView).dll_obj;
            }
        }
