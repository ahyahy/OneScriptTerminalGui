...
namespace ostgui
{
    ...
    [ContextClass("ТфПанельВкладок", "TfTabView")]
    public class TfTabView : AutoContext<TfTabView>
    {
        ...
        public IValue Y
        {
            ...
            set
            {
                ...
                {
                    Base_obj.M_TabView.Y = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_TabView.Y = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        public IValue X
        {
            ...
            set
            {
                if (Utils.IsType<TfPos>(value))
                {
                    Base_obj.M_TabView.X = ((TfPos)value).Base_obj.M_Pos;
                }
                else if (Utils.IsNumber(value))
                {
                    Base_obj.M_TabView.X = Terminal.Gui.Pos.At(Utils.ToInt32(value));
                }
            }
        }
        ...
        [ContextProperty("МаксимальнаяДлинаЗаголовка", "MaxTabTextWidth")]
        public int MaxTabTextWidth
        {
            get { return Base_obj.MaxTabTextWidth; }
            set { Base_obj.MaxTabTextWidth = value; }
        }
        ...
        public IValue SuperView
        {
            get
            {
                if (Base_obj.M_TabView.SuperView.GetType().ToString().Contains("+ContentView"))
                {
                    return Utils.RevertEqualsObj(Base_obj.M_TabView.SuperView.SuperView).dll_obj;
                }
                return Utils.RevertEqualsObj(Base_obj.M_TabView.SuperView).dll_obj;
            }
        }

        [ContextProperty("СмещениеПрокрутки", "TabScrollOffset")]
        public int TabScrollOffset
        {
            get { return Base_obj.TabScrollOffset; }
            set { Base_obj.TabScrollOffset = value; }
        }
        ...
        [ContextProperty("ВкладкаИзменена", "SelectedTabChanged")]
        public TfAction SelectedTabChanged { get; set; }

        [ContextProperty("ВкладкаНажата", "TabClicked")]
        public TfAction TabClicked { get; set; }
